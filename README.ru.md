...
- **Домены, а не только IP.** Имена берутся из SNI рукопожатий TLS и QUIC (HTTP/3) и
  DNS-ответов в трафике, PTR идет запасным вариантом. Найденные имена сохраняются между
  запусками.
