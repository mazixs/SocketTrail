...
выключенным пользовательским unit systemd. В рекомендуемых зависимостях указан
`wireshark-common`. Минимальная версия glibc - та, что на машине сборки: релиз
собирается на Ubuntu 24.04, то есть glibc 2.39.
...
разделом CHANGELOG в описании. GitHub хранит аттестации только для публичных
репозиториев, поэтому, пока репозиторий приватный, этот шаг пропускается.
